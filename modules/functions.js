    ...
    Bot.deployCommands = async (force = false) => {
        ...
        if (force) {
            ...
            try {
                ...
                let currentGuildCommands = await client.shard.broadcastEval(
                ...
                if (newGuildComs?.length || changedGuildComs?.length) {
                    await client.shard.broadcastEval(
                        async (client, { guildId, newGuildComs, changedGuildComs }) => {
                            const targetGuild = await client.guilds.cache.get(guildId);
                            if (targetGuild) {
                ...
                if (Bot.config.enableGlobalCmds) {
                    ...
                    if (changedGlobalComs.length) {
                        ...
                        outLog.push({
                            name: "**Changed Global**",
                            value: changedGlobalComs?.length
                                ? changedGlobalComs.map((diffCom) => ` * ${diffCom.com.name}`).join("\n")
                                : "N/A",
                        });
                    }
                }
...
function checkCmds(newCmdList, oldCmdList) {
    ...
    for (const cmd of newCmdList) {
        ...
        for (const ix in cmd.options) {
            ...
            if (!cmd.options[ix].minValue && !Number.isNaN(cmd.options[ix].minValue)) cmd.options[ix].minValue = undefined;
            if (!cmd.options[ix].maxValue && !Number.isNaN(cmd.options[ix].maxValue)) cmd.options[ix].maxValue = undefined;
            if (!cmd.options[ix].minLength && !Number.isNaN(cmd.options[ix].minLength)) cmd.options[ix].minLength = undefined;
            if (!cmd.options[ix].maxLength && !Number.isNaN(cmd.options[ix].maxLength)) cmd.options[ix].maxLength = undefined;
            ...
            for (const opt of Object.keys(cmd.options[ix])) {
                ...
                if (opt === "choices") {
                    ...
                    if (
                        (newOpt.required !== thisOpt.required && (newOpt.required || thisOpt.required)) ||
                        ...
                        (newOpt.minValue !== thisOpt.minValue && (!Number.isNaN(newOpt?.minValue) || !Number.isNaN(thisOpt?.minValue))) ||
                        (newOpt.maxValue !== thisOpt.maxValue && (!Number.isNaN(newOpt?.maxValue) || !Number.isNaN(thisOpt?.maxValue))) ||
                        (newOpt.minLength !== thisOpt.minLength &&
                            (!Number.isNaN(newOpt?.minLength) || !Number.isNaN(thisOpt?.minLength))) ||
                        (newOpt.maxLength !== thisOpt.maxLength &&
                            (!Number.isNaN(newOpt?.maxLength) || !Number.isNaN(thisOpt?.maxLength))) ||
                    ...
                    if (thisOpt?.type === ApplicationCommandOptionType.Subcommand) {
                        for (const optIx in thisOpt.options) {
                            ...
                            if (
                                (newSubOpt.required !== thisSubOpt.required && (newSubOpt.required || thisSubOpt.required)) ||
                                (newSubOpt.name !== thisSubOpt.name && (newSubOpt.name || thisSubOpt.name)) ||
                                (newSubOpt.autocomplete !== thisSubOpt.autocomplete &&
                                    (newSubOpt.autocomplete || thisSubOpt.autocomplete)) ||
                                (newSubOpt.description !== thisSubOpt.description && (newSubOpt.description || thisSubOpt.description)) ||
                                (newSubOpt.minValue !== thisSubOpt.minValue &&
                                    (!Number.isNaN(newSubOpt?.minValue) || !Number.isNaN(thisSubOpt?.minValue))) ||
                                (newSubOpt.maxValue !== thisSubOpt.maxValue &&
                                    (!Number.isNaN(newSubOpt?.maxValue) || !Number.isNaN(thisSubOpt?.maxValue))) ||
                                (newSubOpt.minLength !== thisSubOpt.minLength &&
                                    (!Number.isNaN(newSubOpt?.minLength) || !Number.isNaN(thisSubOpt?.minLength))) ||
                                (newSubOpt.maxLength !== thisSubOpt.maxLength &&
                                    (!Number.isNaN(newSubOpt?.maxLength) || !Number.isNaN(thisSubOpt?.maxLength))) ||
